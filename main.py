...
def spin_wheel(authorization):
    ...
    while True:
        response = requests.post(url_spin_wheel, headers=headers, json=spin_data)

        if response.status_code == 200 and response.json().get("ok"):
            result = response.json().get("result", {})
            spin_reward = result.get("reward", 0)
            balance = result.get("balance", 0)
            toncoin = result.get("toncoin", 0)
            notcoin = result.get("notcoin", 0)
            ticket = result.get("tickets", 0)

            print(f"#SPIN - REWARD {spin_reward}")
            print(f"TICKETS: {ticket}")
            print(f"TOTAL TON COIN: {toncoin}")
            print(f"TOTAL NOT COIN: {notcoin}")
            print(f"BALANCE: {balance}")
            
            # Jika tiket habis, keluar dari loop
            if ticket == 0:
                print("TIKET KAMU HABIS! COBA LAGI LAIN KALI...")
                break
            else:
                print("TUNGGU 5 DETIK UNTUK SPIN SELANJUTNYA...")
                time.sleep(5)
        else:
            print("#SPIN - GAGAL SPIN!")
            break
